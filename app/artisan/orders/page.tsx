...
import { Package, Search, ExternalLink, ChevronLeft, Image as ImageIcon } from "lucide-react";
import { artisanApi, API_URL } from "@/lib/api";
...
export default function ArtisanOrdersPage() {
  ...
  const fetchOrders = (artisanId: number) => {
    setLoading(true);
    artisanApi.getDashboard(artisanId)
      .then(data => {
  ...
  useEffect(() => {
    if (!session) return;
    fetchOrders(session.artisan_id!);
  }, [session]);
  ...
  const handleUpdateStatus = async (orderId: number, currentStatus: string) => {
    ...
    try {
      await artisanApi.updateOrderStatus(orderId, {
        status: "shipped",
