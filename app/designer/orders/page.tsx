...
import { designerApi, API_URL } from "@/lib/api";
...
export default function DesignerOrdersPage() {
  ...
  const fetchOrders = (designerId: string) => {
    setLoading(true);
    designerApi.getDashboard(designerId)
      .then((data) => {
  ...
  useEffect(() => {
    if (!session) return;
    fetchOrders(session.user_id);
  }, [session]);

  const handleUpdateStatus = async (orderId: number, currentStatus: string) => {
    const trackingNumber = prompt(locale === "en" ? "Enter tracking number to confirm shipment:" : "กรุณากรอกหมายเลขพัสดุ (Tracking Number) เพื่อยืนยันการจัดส่ง:");
    if (!trackingNumber || !session) return;

    try {
      await designerApi.updateOrderStatus(orderId, {
        status: "shipped",
  ...
  return (
    <div className="min-h-screen bg-stone-50 pb-20">
      <div className="bg-white border-b border-brand-200 pt-28 pb-10 px-4 text-center">
        <h1 className="text-3xl font-bold text-brand-950 thai-serif">
          {locale === "en" ? "Manage orders" : "จัดการออเดอร์"}
        </h1>
        <p className="text-brand-950/60 text-xs mt-2 max-w-sm mx-auto leading-relaxed">
          {locale === "en" ? "Review orders, confirm payment, and manage shipment." : "ตรวจสอบคำสั่งซื้อ ยืนยันการชำระเงิน และจัดการการจัดส่งสินค้า"}
        </p>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4 mt-8">
        ...
        {loading ? (
          <div className="flex justify-center p-10"><div className="w-8 h-8 border-2 border-brand-900 border-t-transparent rounded-full animate-spin" /></div>
        ) : orders.length === 0 ? (
          <div className="bg-white rounded-3xl p-12 text-center border border-amber-100/50 shadow-sm">
            ...
            <p className="text-brand-600">{locale === "en" ? "No orders yet." : "ยังไม่มีรายการสั่งซื้อ"}</p>
          </div>
        ) : (
            ...
            <table className="w-full text-left text-sm">
              <thead className="bg-brand-50 text-brand-900">
                <tr>
                  ...
                  <th className="px-4 py-3 font-semibold">{locale === "en" ? "Product" : "สินค้า"}</th><th className="px-4 py-3 font-semibold">{locale === "en" ? "Customer" : "ลูกค้า"}</th><th className="px-4 py-3 font-semibold">{locale === "en" ? "Payment" : "ยอดชำระ"}</th><th className="px-4 py-3 font-semibold">{locale === "en" ? "Payment slip" : "สลิปการโอน"}</th><th className="px-4 py-3 font-semibold">{locale === "en" ? "Status" : "สถานะ"}</th><th className="px-4 py-3 font-semibold">{locale === "en" ? "Action" : "การกระทำ"}</th>
                </tr>
              </thead>
                  ...
                  <tr key={o.id} className="hover:bg-amber-50/30 transition-colors">
                    ...
                    <td className="px-4 py-4">
                      {o.status === "processing" && (
                        ...
                        >
                          {locale === "en" ? "Ship order" : "จัดส่งสินค้า"}
                        </button>
                      )}
                    </td>
                  </tr>
