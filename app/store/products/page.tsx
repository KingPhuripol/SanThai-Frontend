  ...
  const fetchProducts = async () => {
    ...
    try {
      ...
      if (session && session.artisan_id) {
        const data = await productsApi.list({ artisan_id: session.artisan_id });
        setProducts(data);
      }
    } catch (err) {
      ...
      <div className="max-w-[1000px] mx-auto px-4 md:px-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-brand-950 thai-serif">จัดการสินค้า</h1>
              <p className="text-brand-900/60 text-sm">แก้ไข ลบ หรือเพิ่มสินค้าใหม่</p>
            </div>
          </div>
          <Link
                ...
                {products.map((product) => (
                  <tr key={product.id} className="border-b border-brand-100/50 hover:bg-brand-50/50 transition-colors">
                    <td className="p-4 pl-6">
                      <div className="flex items-center gap-4">
                        {product.images && product.images.length > 0 ? (
                          <img src={product.images[0]} alt={product.title_th} className="w-16 h-16 rounded-lg object-cover border border-brand-100" />
                        ) : (
                        ...
                        <span className="font-bold text-brand-900">{product.title_th}</span>
                      </div>
                    </td>
                    ...
                    <td className="p-4 pr-6 text-right space-x-2">
                      <button 
                        onClick={() => alert('ฟังก์ชันแก้ไขกำลังพัฒนา (1.5 - 2 ชม.)')}
